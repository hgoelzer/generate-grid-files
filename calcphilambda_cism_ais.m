...
af2_file_name = ['af2_CISM_AIS_' sprintf('%05d',res*1000) 'm.nc'];
...
for ip=1:nx
    xd(ip) = (dx+(round(res*1000/2)) + (ip-1) * res*1000);
end
for jp=1:ny
    yd(jp) = (dy+(round(res*1000/2)) + (jp-1) * res*1000);
end
...
for ip=1:nx
  for jp=1:ny
    ...
    x = (dx+(round(res*1000/2)) + (ip-1) * res*1000);
    y = (dy+(round(res*1000/2)) + (jp-1) * res*1000);
...
ncwriteatt(af2_file_name,'/','Description','Area scaling factor (af2) for CISM Antarctic grid. Multiply with 2D data to correct the projection error. af2=(1/k)^2, where k is the map scale factor. Calculated after Snyder (1987) by Heiko Goelzer, NORCE, June 2022.')

% Mapping information
mapping = 'mapping';
nccreate(af2_file_name,'mapping','Datatype','int32');
...
ncwriteatt(af2_file_name,'mapping', 'semi_major_axis', 6378137.) ;
ncwriteatt(af2_file_name,'mapping', 'inverse_flattening', 298.2572) ;
