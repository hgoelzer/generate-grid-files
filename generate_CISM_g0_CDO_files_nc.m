...
[LI_grid_center_lat,LI_grid_center_lon]=polarstereo_inv(...
    xcenters(:)-proj_info.falseeasting+round(dx),...
    ycenters(:)-proj_info.falsenorthing+round(dy),...
...
[LI_grid_corner_lat,LI_grid_corner_lon]=polarstereo_inv(...
    xcorners(:)-proj_info.falseeasting+round(dx),...
    ycorners(:)-proj_info.falsenorthing+round(dy),...
    proj_info.earthradius,proj_info.eccentricity,...
    proj_info.standard_parallel,...
    proj_info.longitude_rot);
...
%% write 2D and 1d x,y
wnc(xcenters-proj_info.falseeasting+round(dx),grid.xyOutputFileName,'x02','m','grid center x-coordinate',{'x0','y0'},0,'classic')
wnc(ycenters-proj_info.falsenorthing+round(dy),grid.xyOutputFileName,'y02','m','grid center y-coordinate',{'x0','y0'},0,'classic')

wnc(squeeze(xcenters(:,1))-proj_info.falseeasting+round(dx),grid.xyOutputFileName,'x0','m','grid center x-coordinate','x0',0,'classic')
wnc(squeeze(ycenters(1,:))-proj_info.falsenorthing+round(dy),grid.xyOutputFileName,'y0','m','grid center y-coordinate','y0',0,'classic')
